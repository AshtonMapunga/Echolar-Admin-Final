// business_software_flow.js - WhatsApp Chatbot Business Software Flow (Updated with API Integration)
const axios = require('axios');

class BusinessSoftwareFlow {
    constructor() {
        // Business Software sub-services states
        this.BUSINESS_SOFTWARE_STATES = {
            SAGE_OPTION: 'sage_option',
            ZOHO_OPTION: 'zoho_option',
            BOOKKEEPER_OPTION: 'bookkeeper_option',
            QUICKPRO_PAYROLL_OPTION: 'quickpro_payroll_option',
            PRICING_INFO: 'pricing_info',
            COLLECT_COMPANY_NAME: 'collect_company_name_software',
            COLLECT_CONTACT_NAME: 'collect_contact_name_software',
            COLLECT_EMAIL: 'collect_email_software',
            COLLECT_PHONE: 'collect_phone_software',
            CONFIRM_APPLICATION: 'confirm_application_software',
            BUSINESS_SOFTWARE_END: 'business_software_end'
        };

        // Pricing information for each service
        this.PRICING = {
            'SAGE Evolution': {
                'Yearly - Standard Modules': '$1200',
                'Yearly – Medium': '$2000',
                'Monthly': '$100'
            },
            'Bookkeeper': {
            ...
            'Zoho': {
                'Varies': 'Contact for custom pricing based on modules and users'
            }
        };
    ...
    debug(message, data = null) {
        ...
        console.log(`[${timestamp}] BUSINESS_SOFTWARE_FLOW: ${message}`);
        if (data) {
            console.log(`[${timestamp}] DATA:`, JSON.stringify(data, null, 2));
        }
    }

    // Check if current state is a business software state
    isBusinessSoftwareState(state) {
        return Object.values(this.BUSINESS_SOFTWARE_STATES).includes(state);
    }

    // Start business software sub-service
    startBusinessSoftwareSubService(session, phoneNumber, subServiceName) {
        session.selectedSubService = subServiceName;
        session.state = this.BUSINESS_SOFTWARE_STATES.PRICING_INFO;
        session.applicationData = {}; // Initialize application data

        this.debug('Starting Business Software sub-service', {
            phoneNumber,
    ...
    // Process business software input
    async processBusinessSoftwareInput(message, session, phoneNumber) {
        const userInput = message.trim();
        this.debug('Processing Business Software input', {
            phoneNumber,
        ...
        if ((userInput.toLowerCase() === 'proceed' || userInput.toLowerCase() === 'apply') &&
            ...
            session.state = this.BUSINESS_SOFTWARE_STATES.COLLECT_COMPANY_NAME;
            this.debug('Business Software application started', { phoneNumber });
        ...
        switch (session.state) {
            case this.BUSINESS_SOFTWARE_STATES.COLLECT_COMPANY_NAME:
                session.applicationData.companyName = userInput;
                session.state = this.BUSINESS_SOFTWARE_STATES.COLLECT_CONTACT_NAME;
                return {
                    type: 'message',
                    content: `Thank you! Now please provide your *Full Name*:`
                };

            case this.BUSINESS_SOFTWARE_STATES.COLLECT_CONTACT_NAME:
                session.applicationData.contactName = userInput;
                session.state = this.BUSINESS_SOFTWARE_STATES.COLLECT_EMAIL;
                return {
                    type: 'message',
                    content: `Great! Now please provide your *Email Address*:`
                };

            case this.BUSINESS_SOFTWARE_STATES.COLLECT_EMAIL:
                ...
                session.state = this.BUSINESS_SOFTWARE_STATES.COLLECT_PHONE;
                return {
                    type: 'message',
                    content: `Perfect! Finally, please provide your *Phone Number*:`
                };

            case this.BUSINESS_SOFTWARE_STATES.COLLECT_PHONE:
                session.applicationData.phoneNumber = userInput;
                session.state = this.BUSINESS_SOFTWARE_STATES.CONFIRM_APPLICATION;
...
Please confirm if this information is correct by typing 'confirm' to submit your application, or 'cancel' to start over.
            ...
            case this.BUSINESS_SOFTWARE_STATES.CONFIRM_APPLICATION:
                if (userInput.toLowerCase() === 'confirm') {
                    ...
                    try {
                        ...
                        if (applicationResult.success) {
                            session.state = this.BUSINESS_SOFTWARE_STATES.BUSINESS_SOFTWARE_END;
                            session.applicationData.applicationId = applicationResult.applicationId;

                            return {
                                type: 'message',
                                content: `✅ *Application Submitted Successfully!*\n\n📋 **Application Details:**\n• Service: ${session.selectedSubService}\n• Company: ${session.applicationData.companyName}\n• Contact: ${session.applicationData.contactName}\n• Reference: ${applicationResult.referenceNumber || 'Pending'}\n\nThank you for your application. Our software specialists will contact you within 24 hours at ${session.applicationData.phoneNumber} or ${session.applicationData.email} to discuss your software needs.\n\nType 'menu' to return to the main menu or 'back' to return to Business Software services.`
                            };
                        } else {
        ...
        if (userInput.toLowerCase() === 'back') {
            // Go back to Business Software main menu
            session.state = 'business_software';
            session.selectedSubService = null;
            session.applicationData = {};

            return {
                type: 'template',
                templateSid: 'HX8404ac62c493dfbb97301e2f35a21fb4', // Business Software template
                variables: {}
            };
        }
        ...
        // Default response for Business Software states
        const serviceName = session.selectedSubService || 'Business Software Service';
        return {
            type: 'message',
            content: `📋 You're in the *${serviceName}* section.\n\nPlease use the interactive buttons or:\n• Type 'proceed' to start application\n• Type 'back' to return to Business Software services\n• Type 'menu' for main menu\n• Type 'help' for assistance`
        };
    }
    ...
    async createApplicationViaAPI(session, phoneNumber) {
        try {
            ...
            const applicationData = {
                ...
                serviceType: session.selectedSubService || 'Business Software Service',
                status: 'Pending',
            ...
            this.debug('Calling API to create business software application', {
            ...
            if (response.data.success) {
                this.debug('Business software application created successfully via API', {
    ...
    // Get business software summary for admin/debugging
    getBusinessSoftwareSummary(session) {
        if (!this.isBusinessSoftwareState(session.state)) return null;

        return {
            currentService: session.selectedSubService || 'Business Software Service',
            currentState: session.state,
    ...
    // Calculate progress through business software flow
    calculateProgress(session) {
        if (!this.isBusinessSoftwareState(session.state)) return 0;

        const stateProgress = {
            [this.BUSINESS_SOFTWARE_STATES.SAGE_OPTION]: 10,
            [this.BUSINESS_SOFTWARE_STATES.ZOHO_OPTION]: 10,
            [this.BUSINESS_SOFTWARE_STATES.BOOKKEEPER_OPTION]: 10,
            [this.BUSINESS_SOFTWARE_STATES.QUICKPRO_PAYROLL_OPTION]: 10,
            [this.BUSINESS_SOFTWARE_STATES.PRICING_INFO]: 20,
            [this.BUSINESS_SOFTWARE_STATES.COLLECT_COMPANY_NAME]: 40,
            [this.BUSINESS_SOFTWARE_STATES.COLLECT_CONTACT_NAME]: 60,
            [this.BUSINESS_SOFTWARE_STATES.COLLECT_EMAIL]: 80,
            [this.BUSINESS_SOFTWARE_STATES.COLLECT_PHONE]: 90,
            [this.BUSINESS_SOFTWARE_STATES.CONFIRM_APPLICATION]: 95,
            [this.BUSINESS_SOFTWARE_STATES.BUSINESS_SOFTWARE_END]: 100
        };
...
module.exports = BusinessSoftwareFlow;